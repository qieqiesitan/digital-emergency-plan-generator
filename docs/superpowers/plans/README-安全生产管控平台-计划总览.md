...
| 3 | `2026-09-17-major-hazard-frontend.md` | P0-6 前端 | 计划 1 | ✅ 已写（7 任务） |
| 4 | `2026-09-17-major-hazard-report.md` | P0-7 报告导出 | 计划 1、3 | ✅ 已写（3 任务） |
| 5 | `2026-09-17-datahub-ingest.md` | P1 数据接入适配层 | 计划 2 | ✅ 已写（7 任务） |
| 6 | `2026-09-17-ai-extraction-pipeline.md` | P0-5 后半（资料 → 台账） | 计划 2、5 | ✅ 已写（6 任务） |
| 7 | `2026-09-17-cross-module-linkage.md` | P1 关联打通（风险/隐患 ↔ 重大危险源/预案） | 计划 1 | ✅ 已写（5 任务） |
| 8 | `2026-09-17-work-ticket-core.md` | P2-0 + P2-a（GB 30871 清洗 + 审批引擎 + 动火/受限空间） | 计划 2、3 | ✅ 已写（8 任务） |
| 9 | `2026-09-17-work-ticket-8types.md` | P2-b（其余 6 类 + 多单位会签） | 计划 8 | ✅ 已写（6 任务） |
| 10 | `2026-09-17-platform-polish.md` | P3（AI 能力注册表 + 调用统计 + 跨企业总览） | 计划 2 | ✅ 已写（4 任务） |

**全部 10 份计划已写完**，合计 10 个文件、50 个任务、~10500 行。计划 1 已实现并合入 master；计划 2~10 待执行。

**执行前必读的三条前置（否则会踩坑）**：

1. **计划 8 的任务 1 是前置且不可跳过**——GB 30871 文本有系统性 OCR 讹字（"式"0 次 / "怯"30 次、"Ⅱ"0 次 / "聂"4 次），不修就会在动火票上印出错字
2. **计划 9 必须在计划 8 验证通过后开工**——骨架没跑通就铺 6 类等于 6 次返工
3. **需要真库的验证在 worktree 里做不了**——容器挂载的是主仓库，做法是先完成全部纯代码任务并全绿、合并回 master 后再一次性补真库验证（§4.3）
