...
3a. **图例不做几何聚类自动剔除**（2026-08-07 用户反馈后修订）：密集分区图上"紧邻多色小色块"会被并查集误判为图例整组剔除；改为保留为普通分区，由用户在预览中删除（OCR 佐证的图例识别列为后续可选增强）。
4. 自动剔除项在预览中展示并可一键恢复。
...
- 用途：
  ...
  2. **图例佐证**（预留）：图例色块附近的"重大/较大/一般/低"等文字可用于后续图例识别增强（当前几何聚类已停用）。
...
1. 合成四色图例（4 个紧邻小色块）→ 保留为普通分区，`excluded` 为空。
...
5a. 图例小色块 → **保留为普通分区**（不再几何剔除；`excluded` 为空）。
6. 既有透视/噪点/文字用例保持通过。
...
- ~~图例簇判定可能把紧邻的多个真实小分区误判为图例~~（2026-08-07 已停用该规则，图例保留为分区由人工处理）。
