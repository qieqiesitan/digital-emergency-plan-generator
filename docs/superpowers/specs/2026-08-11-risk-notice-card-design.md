...
| 其他伤害 | — / — / — / 紧急出口（默认组；仅通用疏散提示，不配生产性防护标志） |

匹配规则：`accident_type` 精确匹配映射表；自定义/非标准类型先查 `EXTRA_SIGN_GROUPS`（如「火灾爆炸」→ 火灾组）；仍未命中（含空值）→ **默认组**（仅紧急出口，避免给会议室/办公区等非生产场景误配安全帽等防护标志）。一个风险点含多类事故时合并去重，按 警告→禁止→指令→提示 排序，每类最多取 2 个。
