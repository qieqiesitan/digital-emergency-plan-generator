...
| 15 | AI 辅助配置（补审） | 7 项 AI 辅助全部纳入（组织 AI 建树 / 计划一键生成 / AI 检查表 / AI 治理方案 / 双等级参数建议 / 隐患登记 AI 摘要分类 / 智能引导向导）；**全部文本通道，不依赖图像识别** |
| 16 | 二期 | 未闭环重大隐患写入预案生成、监管平台真实对接、岗位/班组风险告知卡、拍照识别隐患（需多模态模型）（本次不做） |
...
- **AI 建树**：`POST /enterprises/{id}/org/ai-suggest`——输入企业基础信息 + 现有 org_structure + 行业/人数，返回组织树与成员草稿（§3.7 #1）；前端分步确认，成员邮箱留空待补。
...
### 3.7 AI 辅助配置（文本通道，用户已确认全量纳入）

**原则（与全系统一致）**：

- **全部文本进/出**：只读企业已有结构化数据与用户文字输入，不调用图像/多模态识别（项目无视觉模型，二期拍照识别另行评估）；
- **AI 预填 + 人工确认**：所有 AI 生成内容标记来源（ai / manual），确认后才落库；
- **失败兜底**：AI 超时/不可用时返回「无 AI 建议」，流程照常手动运行；
- **系统默认优先**：折算系数、层级映射、判定要点、期限等专业参数默认用系统字典值，企业无需配置即可用。

**7 项 AI 辅助及归属**：

| # | 能力 | 输入（仅文本） | 输出 | 归属 |
|---|------|----------------|------|------|
| 1 | 组织架构 AI 建树 | 企业基础信息 + 现有 org_structure + 行业/人数 | 部门→班组→岗位树 + 成员草稿（姓名/岗位/角色；邮箱留空待企业补） | §3.5 |
| 2 | 排查计划 AI 一键生成 | 问答（区域清单/频次偏好） | 整套排查计划（名称/频次/责任人建议） | §6 |
| 3 | AI 检查表生成 | 行业 + 风险点/措施 | 检查表核对项（含 expected_note），默认模板兜底 | §7 |
| 4 | AI 治理方案草稿 | 隐患描述 + 判定要点 + 关联措施 | 治理方案（goal/measures/budget/emergency/acceptance） | §9 |
| 5 | 双等级参数 AI 建议 | 事件描述 + 管控措施文本 | 固有/现有参数与等级建议（A 规格 §5.2 方式三） | A 规格 |
| 6 | 隐患登记 AI 摘要/分类 | 登记描述文字 | title 摘要 + hazard_type + 分级建议 | §8 |
| 7 | 智能引导向导 | 问答（行业/区域/人数/频次偏好） | 一次性预填 组织→排查计划→检查表，分步确认 | §3.8 |

### 3.8 智能引导向导（Onboarding 扩展）

- 新企业首次进入「隐患排查治理」Tab（或企业详情未完成初始配置时）显示向导入口；
- 步骤：①基础问答（行业/主要区域/大概人数/希望排查频次）→ ②AI 预填组织树与成员草稿 → ③AI 预填排查计划与检查表 → ④确认落库（分步可跳过/稍后补）；
- 复用现有 Onboarding 组件模式（StepOrg 等）与 `llm_text_completion`；向导进度可中断、可从「智能引导」入口重进。
...
- **AI 一键生成**：`POST /ai/plan-builder`——问答输入（区域清单/频次偏好）→ 返回整套计划列表（名称/类别/频次/责任人建议/覆盖分区），页面整批确认或逐条调整后落库（§3.7 #2）；
...
- **AI 检查表生成**：`POST /ai/checklist-template`——输入行业 + 风险点/措施文本，返回核对项列表（content/expected_note），确认后存为企业模板（§3.7 #3）；
...
- **AI 摘要/分类**：Web 登记时点「AI 智能填写」——输入描述文字后自动生成 title 摘要、hazard_type、分级建议（§3.7 #6；仅文本，不读照片），人工确认后填入；
...
- **AI 治理方案草稿**：`POST /ai/governance-plan`——输入隐患描述 + 判定要点 + 关联措施文本，返回治理方案草稿（§3.7 #4），人工修改确认后填入；
...
| POST | `/enterprises/{id}/hazard-inspection/ai/plan-builder` | AI 一键生成排查计划 |
| POST | `/enterprises/{id}/hazard-inspection/ai/checklist-template` | AI 检查表生成 |
| POST | `/enterprises/{id}/hazard-inspection/ai/governance-plan` | AI 治理方案草稿 |
| POST | `/enterprises/{id}/hazard-inspection/ai/record-assist` | 隐患登记 AI 摘要/分类 |
| POST | `/enterprises/{id}/hazard-inspection/ai/setup-wizard` | 智能引导：预填组织→计划→检查表 |
| POST | `/enterprises/{id}/org/ai-suggest` | 组织架构 AI 建树（§3.5） |
...
- **pytest**：
  ...
  - AI 辅助：7 项接口全部 mock LLM——正常返回/超时降级/非法 JSON 三类用例；确认后落库、来源标记（ai/manual）；
...
5. 7 项 AI 辅助可用：预填 + 人工确认，AI 失败降级不阻塞，生成内容标记来源；
6. 一般/重大流程 × 标准/严格模式行为正确（含退回、二次复核）；
7. 超期预警与到期前提醒、审批待办通知生效且不重复（角标正确）；
8. 隐患闭环后风险点「未闭环隐患」标记消失，告知卡/清单实时反映；
9. 驾驶舱数据正确（含类型分布），台账与监管上报台账可导出；
10. 公示页/公开页无敏感信息；
11. 全部门禁通过。
