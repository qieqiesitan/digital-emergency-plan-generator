...
3. 生产库实测（库 `emergency_plan`）存在**两种**存量形态：
   - 树格式（西安宝岳空间科技有限公司，37 节点）：其中 **29 个是应急节点**（`preset-org-root` 根 + 6 个 `preset-*` 应急小组及其 18 个岗位 + `node-5` 部门型「应急指挥部」及其 3 个岗位「总指挥/副总指挥/成员」），**8 个是真实公司节点**（`公司` → `应急安全事业部`/`管网事业部`/`资规事业部` → `技术开发` + 3 个「部门经理」岗位）。`enterprise_members` 3 名成员（刘昕野/总经理、赵志龙/项目总监、程磊/项目经理）的 `org_node_id` 分别指向 `preset-headquarters-0/1/2`，即「应急指挥部/总指挥」「副总指挥」「成员」。公司职位与应急角色被压在同一棵树上。
...
|---|---|---|---|
| 西安宝岳空间科技有限公司 | 37 节点（29 应急 + 8 公司） | 8 个公司节点（公司 → 三个事业部 → 技术开发 + 3 个部门经理） | 顶层「应急组织机构」+ 7 个子单元（6 个小组 + 1 个同名部门型「应急指挥部」）+ 21 个角色；3 条指派：总指挥=刘昕野、副总指挥=赵志龙、成员=程磊 |
| 延长壳牌石油有限公司（西安明光路加油站） | 25 节点（全部为未编辑的完整预置） | 0 个，需用户补建公司架构 | 顶层 + 6 个小组 + 18 个角色，无指派 |
| 陕西宝岳测绘有限公司 | 6 条旧分组、28 名内嵌成员 | 0 个 | 顶层「应急组织机构」+ 6 个小组 + 21 个派生角色；新建 28 条 `enterprise_members` 与 28 条指派 |
