...
export default function StepRiskChemical({
  ...
  const generate = async () => {
    setGenerating(true);
    try {
      const resp = await generateChemicalsAI(
        enterpriseId,
        [
  ...
  const accept = async (item: CandidateItem) => {
    try {
      ...
      const created = await batchCreateChemicals(enterpriseId, [toCreatePayload(item)], { skipGlobalError: true });
  ...
  const acceptAll = async () => {
    ...
    try {
      const created = await batchCreateChemicals(enterpriseId, items.map(toCreatePayload), { skipGlobalError: true });
  ...
  const unacceptAll = async () => {
    ...
    try {
      for (const item of items) {
        ...
        if (id) await deleteChemical(enterpriseId, id, { skipGlobalError: true });
      }
      setAccepted([]);
      // 移回候选区，可重新编辑后再采纳
