...
export default function RiskAssessmentTab({ enterpriseId }: Props) {
  ...
  const loadReport = async () => {
    try {
      setLoading(true);
      const r = await getRiskAssessment(enterpriseId);
      setReport(r);
    } catch {
      setReport(null);
    } finally {
    ...
    return (
      <div style={{ textAlign: "center", padding: "60px 0" }}>
        <Empty
          ...
          description={
            <>
              <Title level={4}>尚未生成风险评估报告</Title>
  ...
  return (
    <div>
      <Space style={{ marginBottom: 16 }}>
        ...
        <Button
          icon={<EyeOutlined />}
          onClick={() => navigate(`/enterprises/${enterpriseId}/risk-assessment/preview`)}
        >
          预览
        </Button>
      </Space>
