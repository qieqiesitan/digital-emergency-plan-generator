...
export default function RiskAssessmentScreen() {
  ...
  const { data: report, isLoading, error } = useQuery({
    queryKey: ["risk-assessment", enterpriseId],
    queryFn: () => getRiskAssessment(enterpriseId!),
  ...
  const handleGenerate = useCallback(() => {
    ...
    const controller = generateRiskAssessmentStream(
      enterpriseId,
      undefined,
      (event: SSEEvent) => {
  ...
  const handleDownload = async () => {
    if (!enterpriseId) return;
    try {
      await downloadRiskAssessment(enterpriseId);
      ...
      <div className="flex-1 overflow-y-auto" ref={contentRef}>
        ...
        {error && genStatus === "idle" && (
          <EmptyState
            ...
            title="加载失败"
            description="无法加载风险评估报告"
            action="重试"
            onAction={() => queryClient.invalidateQueries({ queryKey: ["risk-assessment", enterpriseId] })}
          />
        )}
        ...
        {!isLoading && !error && !displayContent && genStatus === "idle" && (
          <EmptyState
            ...
            title="暂无风险评估报告"
            description="基于企业风险源数据，AI 将自动生成风险评估报告"
          />
        )}
