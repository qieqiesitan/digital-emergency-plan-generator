...
export const createRecord = (eid: string, data: HazardRecordCreate) =>
  api.post<ApiResponse<HazardRecord>>(`${BASE(eid)}/records`, data, { skipGlobalError: true }).then(r => r.data.data);
...
) => api.post<ApiResponse<HazardRecord>>(`${BASE(eid)}/records/${rid}/grade`, data, { skipGlobalError: true }).then(r => r.data.data);
export const approveRecord = (
  eid: string,
  rid: string,
  data?: { comment?: string | null; rectification_user_id?: string | null },
) => api.post<ApiResponse<HazardRecord>>(`${BASE(eid)}/records/${rid}/approve`, data ?? {}, { skipGlobalError: true }).then(r => r.data.data);
export const rejectRecord = (
  eid: string,
  rid: string,
  data?: { comment?: string | null },
) => api.post<ApiResponse<HazardRecord>>(`${BASE(eid)}/records/${rid}/reject`, data ?? {}, { skipGlobalError: true }).then(r => r.data.data);
export const rectifyRecord = (
  eid: string,
  rid: string,
  data: { content: string; evidence?: string[]; reviewer_user_id: string },
) => api.post<ApiResponse<HazardRecord>>(`${BASE(eid)}/records/${rid}/rectify`, data, { skipGlobalError: true }).then(r => r.data.data);
export const reviewRecord = (
  eid: string,
  rid: string,
  data: { result: "pass" | "fail"; comment?: string | null; evidence?: string[] },
) => api.post<ApiResponse<HazardRecord>>(`${BASE(eid)}/records/${rid}/review`, data, { skipGlobalError: true }).then(r => r.data.data);
export const closeRecord = (
  eid: string,
  rid: string,
  data?: { comment?: string | null },
) => api.post<ApiResponse<HazardRecord>>(`${BASE(eid)}/records/${rid}/close`, data ?? {}, { skipGlobalError: true }).then(r => r.data.data);
...
export const createHazardPlan = (eid: string, data: HazardInspectionPlanCreate) =>
  api.post<ApiResponse<HazardInspectionPlan>>(`${BASE(eid)}/plans`, data, { skipGlobalError: true }).then(r => r.data.data);
export const updateHazardPlan = (eid: string, planId: string, data: HazardInspectionPlanUpdate) =>
  api.put<ApiResponse<HazardInspectionPlan>>(`${BASE(eid)}/plans/${planId}`, data, { skipGlobalError: true }).then(r => r.data.data);
export const deleteHazardPlan = (eid: string, planId: string) =>
  api.delete(`${BASE(eid)}/plans/${planId}`, { skipGlobalError: true });
...
) => api.post<ApiResponse<HazardRecord>>(`${BASE(eid)}/tasks/${taskId}/to-record`, data, { skipGlobalError: true }).then(r => r.data.data);
...
export const createHazardTemplate = (eid: string, data: HazardChecklistTemplateCreate) =>
  api.post<ApiResponse<HazardChecklistTemplate>>(`${BASE(eid)}/templates`, data, { skipGlobalError: true }).then(r => r.data.data);
export const updateHazardTemplate = (eid: string, templateId: string, data: HazardChecklistTemplateUpdate) =>
  api.put<ApiResponse<HazardChecklistTemplate>>(`${BASE(eid)}/templates/${templateId}`, data, { skipGlobalError: true }).then(r => r.data.data);
export const copyHazardTemplate = (eid: string, templateId: string) =>
  api.post<ApiResponse<HazardChecklistTemplate>>(`${BASE(eid)}/templates/${templateId}/copy`, {}, { skipGlobalError: true }).then(r => r.data.data);
export const deleteHazardTemplate = (eid: string, templateId: string) =>
  api.delete(`${BASE(eid)}/templates/${templateId}`, { skipGlobalError: true });
...
export const aiRecordAssist = (eid: string, data: { description: string; object_id?: string | null; measure_id?: string | null }) =>
  api.post<ApiResponse<HazardRecordAssistResult>>(`${BASE(eid)}/ai/record-assist`, data, { skipGlobalError: true }).then(r => r.data.data);
export const aiGradeHazard = (eid: string, data: { description: string; judgment_points?: string | null; measures_text?: string | null }) =>
  api.post<ApiResponse<import("@/types/hazard").HazardAiGradeResult>>(`${BASE(eid)}/ai/grade`, data, { skipGlobalError: true }).then(r => r.data.data);
export const aiGovernancePlan = (eid: string, data: { description: string; judgment_points?: string | null; measures_text?: string | null }) =>
  api.post<ApiResponse<HazardGovernancePlanResult>>(`${BASE(eid)}/ai/governance-plan`, data, { skipGlobalError: true }).then(r => r.data.data);
export const aiPlanBuilder = (eid: string, data: { areas: string; frequency_preference: string }) =>
  api.post<ApiResponse<HazardPlanBuilderResult>>(`${BASE(eid)}/ai/plan-builder`, data, { skipGlobalError: true }).then(r => r.data.data);
export const aiScheduleSuggestion = (eid: string, data: { plan_draft: string; zone_risk_hints?: string | null; history_hints?: string | null }) =>
  api.post<ApiResponse<HazardScheduleSuggestionResult>>(`${BASE(eid)}/ai/schedule-suggestion`, data, { skipGlobalError: true }).then(r => r.data.data);
export const aiChecklistSuggestion = (eid: string, data: { task_context: string }) =>
  api.post<ApiResponse<HazardChecklistSuggestionResult>>(`${BASE(eid)}/ai/checklist`, data, { skipGlobalError: true }).then(r => r.data.data);
export const aiSetupWizard = (eid: string, data: { industry: string; areas: string; employee_count?: string | null; frequency_preference?: string | null }) =>
  api.post<ApiResponse<HazardSetupWizardResult>>(`${BASE(eid)}/ai/setup-wizard`, data, { skipGlobalError: true }).then(r => r.data.data);
export const aiChecklistTemplate = (eid: string, data: { industry: string; risk_points: string }) =>
  api.post<ApiResponse<HazardChecklistTemplateAIResult>>(`${BASE(eid)}/ai/checklist-template`, data, { skipGlobalError: true }).then(r => r.data.data);
