...
CHAT_TOOLS = [
    ...
    {"type": "function", "function": {"name": "list_hazard_templates", "description": "列出隐患排查的**检查表模板**（系统预置 + 指定企业的自定义）。用户问「隐患/排查/检查表模板」时用这个；问「预案模板」才用 list_templates。", "parameters": {"type": "object", "properties": {"enterprise_id": {"type": "string", "description": "企业ID（可选；给了就一并返回该企业的自定义模板）"}}, "required": []}}},
    ...
    {"type": "function", "function": {"name": "search_regulation_articles", "description": "语义检索法规条文原文。当用户询问安全生产、应急管理、消防、职业健康、特种设备、危化品等法律法规问题时，必须调用此工具查找相关法律条文的具体内容和出处。返回条文原文、所属法规全称、文号、条款号；similarity_score 仅向量召回有值（recall=vector），词面召回 recall=lexical 时该字段为 null —— null 不代表不相关，不要把它当成 0 分写进回答。注意：此工具返回的是具体条文，不是法规列表。", "parameters": {"type": "object", "properties": {"query": {"type": "string", "description": "用户问题的关键词或完整句子，用于匹配法规条文"}, "top_k": {"type": "integer", "description": "返回条数，默认8，范围3-15"}}, "required": ["query"]}}},
    ...
    {"type": "function", "function": {"name": "confirm_workflow_step",
     "description": "确认工作流中等待确认的步骤（如生成预案前）。当 get_workflow_progress 显示 paused/等待确认时，用户同意后调用放行继续",
     "parameters": {"type": "object",
                    "properties": {"run_id": {"type": "string", "description": "工作流运行ID（必填）"},
                                   "step_name": {"type": "string", "description": "要确认的步骤名（必填）"}},
                    "required": ["run_id", "step_name"]}}},
    {"type": "function", "function": {"name": "review_plan",
     "description": "审查预案内容质量：返回章节完整性、法规引用真实性、占位符等 issues/warnings。用户要求检查预案质量或工作流审查步骤时调用",
     "parameters": {"type": "object",
                    "properties": {"plan_id": {"type": "string", "description": "预案ID（必填）"}},
                    "required": ["plan_id"]}}},
