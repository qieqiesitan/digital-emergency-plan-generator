...
请提取以下字段（如果文本中没有则标注"未提供"）：
1. 法规编号：如 GB/T 29639-2020、主席令第88号
2. 法规全称
3. 发布机关
4. 发布日期
5. 施行日期
6. 替代关系：替代了哪些旧法规（编号列表）
7. 上位法依据：依据哪些上位法制定（编号列表）
8. 适用主题标签（选填）：应急预案编制/风险评估/应急资源/消防安全/危险化学品/特种设备/职业健康/安全培训/隐患排查...
9. 条文清单：按条款编号分块，每条标注编号和原文
...
async def ai_parse(raw_text: str, ai_config) -> dict:
    ...
    try:
        ...
        api_key = unpad(cipher.decrypt(bytes.fromhex(ai_config.api_key_encrypted)), 16).decode()
    ...
    prompt = PARSE_PROMPT.format(raw_text=raw_text[:30000])
    ...
    async with httpx.AsyncClient(timeout=120) as client:
        ...
        if resp.status_code != 200:
            raise Exception(f"AI解析失败: HTTP {resp.status_code} {resp.text[:300]}")
