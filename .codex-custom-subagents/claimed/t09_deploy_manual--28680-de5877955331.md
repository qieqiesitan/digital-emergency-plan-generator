...
Task: t09_deploy_manual

## 任务：部署可交付性计划任务 9 —— 部署手册 docs/deploy/README-DEPLOY.md

你是一个实现子智能体。严格按以下步骤在指定 worktree 内创建文档并提交。不要修改任务范围之外的文件。不要读计划文件——本任务文件已包含完整任务文本与全部文档内容。
...
把公司部署的全部知识（拓扑、预检、构建、部署、验证、踩坑、回滚）固化为仓库内文档。创建 `docs/deploy/README-DEPLOY.md`，内容必须与任务给定文本逐字一致（注意保持 markdown 代码块的 ``` 语法与表格完整）。

### 步骤 1：创建 docs/deploy/README-DEPLOY.md
...
1. 文件内容与任务给定文本一致（表格 3 列、代码块语法完整、无遗漏章节）；
2. `git diff --check` 干净；
3. 提交只含 `docs/deploy/README-DEPLOY.md`，提交消息精确匹配步骤 2。
