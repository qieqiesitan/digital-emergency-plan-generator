...
Task: cockpit_01_impl_v2

你正在实现「企业驾驶舱重构」实现计划的 任务 1（重试版 v2）：后端驾驶舱聚合服务（纯函数 + 查询编排）。这是 9 个任务中的第 1 个（先做后端）。

背景：上一版（cockpit_01_impl）因计划中 risk_index 公式与测试期望矛盾被 BLOCKED。主控已裁决：**risk_index 采用归一化加权平均口径**（0-100）：`min(100, round((major*100 + larger*70 + general*40 + low*10) / max(total, 1)))`，0 事件时为 0。本任务正文已按新口径修正，请严格按下文执行。

注意：worktree 的 backend 目录下可能残留上一版未提交的两个文件（tests/test_enterprise_cockpit.py 与 app/services/enterprise_cockpit_service.py，内容为旧口径）。请直接以本任务正文为准覆盖/修正这两个文件，然后按 TDD 流程验证并提交。

## 工作目录（重要）
C:\Users\55061\Documents\数字化预案自动生成 2\.worktrees\enterprise-cockpit
（git worktree，分支 codex/enterprise-cockpit。所有命令在 PowerShell 中执行；后端命令用 workdir 进入该目录的 backend 子目录。）
...
创建/覆盖 `backend/tests/test_enterprise_cockpit.py`：
...
预期：FAIL（若旧文件残留，先确认失败来自测试本身，再实现）

步骤 3：实现聚合服务

创建/覆盖 `backend/app/services/enterprise_cockpit_service.py`：
...
## 上下文（场景铺设）
- 这是「企业驾驶舱」功能的后端地基：总览需要一次性聚合风险/隐患/完成度等数据。后续任务 2 会在 enterprises.py 挂 cockpit-summary 端点调用本服务的 build_cockpit_summary。
- 参考既有实现：backend/app/services/risk_stats_service.py（RiskEvent↔RiskUnit/RiskObject→RiskZone 的 join 模式）、backend/app/services/onboarding_service.py（compute_completion 返回 {"percent", "modules":[{key,label,weight,done}]}）。
