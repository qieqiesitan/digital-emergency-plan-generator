...
**默认（本项目约定，2026-08-06 用户指定）：所有角色统一使用 `deepseek-v4-flash`。**

- 实现子智能体：默认 `deepseek-v4-flash`。
- 规格合规审查子智能体：默认 `deepseek-v4-flash`。
- 代码质量审查子智能体：默认 `deepseek-v4-flash`。
- 最终整体审查子智能体：默认 `deepseek-v4-flash`。

原因：`deepseek-v4-pro` 当前环境不可用（调用会报 invalid_request_error）；统一 flash 可保证流程稳定、成本可控。

**例外：** 仅当用户显式指定其他模型时，才按用户指定的模型分派。
